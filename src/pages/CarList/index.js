	...
	const handleCariMobil = (e) => {
		...
		if (savedCars) {
			const filterData = savedCars.filter(
				(items) =>
					items.name.toLowerCase().includes(namaMobil.toLowerCase()) && items.category.includes(kategoriMobil)
			);
			...
			<div className="mt-5 hasil-card">
				...
				{loading ? (
					...
					<div className="d-flex flex-wrap align-items-stretch justify-content-around">
						{mobil.map((result) => {
							return (
								<Card key={result.id} style={{ width: '18rem', margin: '1rem' }} className="card-cont">
									<Card.Img
										variant="top"
										src={
											result.image ? (
									...
									<Card.Body className="d-flex flex-column">
										...
										<IntlProvider locale="id">
											<FormattedNumber value={result.price} style="currency" currency="IDR" /> /
											hari
										</IntlProvider>
										<Card.Text>
										...
										<div className="d-grid mt-auto">
											<Button variant="success" onClick={() => handlePayment(result.id)}>
												Pilih Mobil
											</Button>
										</div>
									</Card.Body>
